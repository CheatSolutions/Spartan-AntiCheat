...
public class PreventionsMenu extends InventoryMenu {

    public static final String title = "Check Preventions";

    public PreventionsMenu() {
            ...
            for (HackType hackType : CheckEnums.HackType.values()) {
                if (hackType.category == categoryType) {
                    ...
                    for (Check.DataType dataType : Check.DataType.values()) {
                        if (!hackType.getCheck().isSilent(dataType, null)) {
    ...
    public boolean internalHandle(PlayerProtocol protocol) {
        ...
        if (item.equals("§4Back")) {
            ...
            if (clickType == ClickType.LEFT) {
                ...
                if (check != null) {
                    check.setSilent(Check.DataType.JAVA, !check.isSilent(Check.DataType.JAVA, null));
                }
                open(protocol);
                ...
                if (check != null) {
                    check.setSilent(Check.DataType.BEDROCK, !check.isSilent(Check.DataType.BEDROCK, null));
                }
                open(protocol);
            }
        }
